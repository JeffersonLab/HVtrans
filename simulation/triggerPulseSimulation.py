...
trigger_rise_time_radian = translationLayer.trigger_rise_time_radian
...
def calc_rise_time_module(polarity): #calculate the current voltage of the rise time linearly, argument boolean polarity
    num_of_steps = (trigger_rise_time_radian / generation_resolution)
...
def calc_trigger(): #generate a trigger pulse according to a PWM waveform 
    ...
    while (int_num_of_modules < num_of_modules):
        ...
        time = 0
        if (polarity == 1):
            while (time < (trigger_duty_cycle * switching_period_radian)):
                trigger.append(trigger_nominal_voltage)
                time = time + generation_resolution
        else: 
            while (time < ((1 - trigger_duty_cycle) * switching_period_radian)):
                trigger.append(0)
                time = time + generation_resolution
        polarity = -polarity
