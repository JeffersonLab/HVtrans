...
def BCM_transient_voltage_calculator():
    time1 = 0
    global BCM_pos_transient_voltage
    global BCM_neg_transient_voltage

    while (time1 < (len(BCM_storage) * BCM_resolution)):
        if (BCM_pos_transient_voltage <= read_current_amplitude_BCM(time1)):
            BCM_pos_transient_voltage = read_current_amplitude_BCM(time1)
        elif (BCM_neg_transient_voltage <= abs(read_current_amplitude_BCM(time1))):
            BCM_neg_transient_voltage = abs(read_current_amplitude_BCM(time1))
...
def calc_total_asymmetry():
    ...
    print(BCM_pos_transient_voltage , BCM_neg_transient_voltage)
    print(detector_pos_transient_voltage , detector_neg_transient_voltage)

    input_asymmetry = (detector_pos_transient_voltage - detector_neg_transient_voltage)

    systematic_error_maybe = (((detector_pos_transient_voltage / BCM_pos_transient_voltage) - (detector_neg_transient_voltage / BCM_neg_transient_voltage)) / ((detector_pos_transient_voltage / BCM_pos_transient_voltage) + (detector_neg_transient_voltage / BCM_neg_transient_voltage)))
